...
def train(verbose=True, keepon=False, it_s=0, has_base=False, path=''):
    ...
    if keepon:
        net.load_state_dict(torch.load(osp.join(cfg.respth,'model_final_'+str(it_s)+'.pth')))
    net.cuda()
    net.train()
    ...
    diter = iter(dl)
    n_epoch = 0
    for it in range(it_s+1,cfg.max_iter,1):
        try:
            im, lb = next(diter)
...
if __name__ == "__main__":
    path = "./res/Cityscapes/model_final_40000.pth"
    train(True,False,0,True,path)
