...
class decoder(nn.Module):
    def __init__(self, num_classes=19, bias=True, dropout_prob=0):
        ...
        self.act2 = nn.PReLU()
        self.final = nn.ConvTranspose2d(48, num_classes, kernel_size=3, stride=4, padding=0, output_padding=1, bias=bias, dilation=1)
    def forward(self,x):
        x1 = self.convt1(x[0])
        x2 = self.convt2(x[1])
        x3 = self.convt3(x[2])
...
if __name__=='__main__':
    ...
    net.load_state_dict(torch.load("res/Cityscapes/model_final.pth"))
    net.cuda()
    net.eval()
    s = time.time()
    res = net(img)
